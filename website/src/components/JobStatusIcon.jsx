...
function JobStatusIconLink(props) {
  return (
    <a key={props.cronJob.namespace+"-"+props.job.name} href={"/namespaces/"+props.cronJob.namespace+"/cronJobs/"+props.cronJob.name+"/jobs/"+props.job.name} data-toggle="tooltip" data-placement="bottom" data-original-title={ props.job.creationTime }>
      <JobStatusIcon status={props.job.status} manual={props.job.manual} />
    </a>
  )
}
...
function JobStatusIcon(props) {
  return (
    ...
    { props.manual ? <path d="M19,31 l0,-10 m0,2 q3,-3 6,0 l 0,8 m0,-8 q3,-3 6,0 l 0,8" stroke="white" strokeWidth="2"/> : null}
    </svg>
  )
}

export { JobStatusIconLink, JobStatusIcon }
